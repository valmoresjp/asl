...
function ModificarCantidad(id){
	...
	//~ console.log(pos);
	//~ $("#ModificarCantidad").offset({top:pos.x,left:pos.y});	
	//~ $("#ModificarCantidad").show();

	//~ console.log($("#ModificarCantidad").position());
...
function CalculoTotalRegistro(id){	
	...
	CalculoTotal()
	datos ={ "accion":"actualizar", "id":parseInt($(id).parent().find("td").eq(0).text()), "datos":cant};
	Cambios(datos);
}
...
function AgregarRegistro(id){
	...
	if ( ID_TABLA_ACTIVA === "#Materiales_prd" ){
		...
		$(tbl + " > tbody > tr > td:nth-child(2)").on("click", function(){
				var input = "<input id='Blistar' type='text' name='Blistar'  onkeyup='Listar(this)' style='width:100%;' placeholder='Ingrese la cantidad..'>";
...
function EliminarRegistro(id){
	...
	Cambios({"accion": "eliminar", "id": r, "datos":-1} );
...
$("#Otros-gastos_prd").click(function(){
	console.log("Se realizo accion sobre la tabla Materiales");
	ID_TABLA_ACTIVA = '#Otros-gastos_prd';
});
